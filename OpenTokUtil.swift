...
class OpenTokUtil: NSObject {
	...
	var kApiKey = "46080142" //"46096372"
	// Replace with your generated session ID
	var kSessionId = "2_MX40NjA5NjM3Mn5-MTUyMzUxNzAxNDU5MX5jZFZoaEFaeHphdkJJdEw5Q3laTFVlOUJ-fg"
	// Replace with your generated token
	var kToken = "T1=="
	...
	func endCall(_ receiverId: Int = 0) {
		var error: OTError?
		defer {
		...
		let dict = ["msg":User.currentUser?.firstName ??  "","loginId":UserUtil.getLoginId() ?? "","chatId":"","recieverLoginId":"\(receiverId)"]
		
		let json = JSON(dict)
		session.signal(withType: SignalTypes.CALL_END, string: json.rawString() ?? "", connection: nil, error: &error)
	...
	func sendMessage(_ message: String, loginId: Int, chatId: Int, recieverId: Int) -> Bool {
		var error: OTError?
		
		let dict = ["msg":message,"loginId":"\(loginId)","chatId":"\(chatId)","recieverLoginId":"\(recieverId)"]
		
		let json = JSON(dict)
		session.signal(withType: SignalTypes.SIGNAL_TYPE_CHAT, string: json.rawString() ?? "", connection: nil, error: &error)
	...
	func call(_ message: String, loginId: Int, chatId: Int, recieverId: Int) {
		var error: OTError?
		let dict = ["msg":message,"loginId":"\(loginId)","chatId":"\(chatId)","recieverLoginId":"\(recieverId)"]
		
		let json = JSON(dict)
		
		session.signal(withType: SignalTypes.SIGNAL_TYPE_CALL, string: json.rawString() ?? "", connection: nil, error: &error)
		print(error?.localizedDescription ?? "")
		_ = doPublish()
	}
	
	func callAccepted(_ dict: [String: Any]) {
		var error: OTError?
//		let dict = ["msg":message,"loginId":"\(loginId)","chatId":"\(chatId)","recieverLoginId":"\(recieverId)"]
		
		let json = JSON(dict)
		
		session.signal(withType: SignalTypes.SIGNAL_TYPE_CALL_ACCEPTED, string: json.rawString() ?? "", connection: nil, error: &error)
		print(error?.localizedDescription ?? "")
	}
	
	func callRejected(_ dict: [String: Any]) {
		var error: OTError?
//		let dict = ["msg":message,"loginId":"\(loginId)","chatId":"\(chatId)","recieverLoginId":"\(recieverId)"]
		
		let json = JSON(dict)
		
		session.signal(withType: SignalTypes.SIGNAL_TYPE_CALL_REJECTED, string: json.rawString() ?? "", connection: nil, error: &error)
		print(error?.localizedDescription ?? "")
	...
	func doPublish()->UIView? {
		...
		return videoRender //publisher?.view
